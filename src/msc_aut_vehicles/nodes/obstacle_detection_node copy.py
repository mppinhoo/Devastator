    ...
    def lidar_callback(self, msg):
        ...
        if min_range1 < self.StopDist:
            if current_cmd_vel_msg.linear.x > 0.05 and -math.pi/3 < min_angle1 < math.pi/3:
                ...
                cmd_vel_msg.linear.x = current_cmd_vel_msg.linear.x * min_range1
                
                if min_angle1 > 0:
                    cmd_vel_msg.angular.z = (-math.pi + min_angle1) * 0.5
                elif min_angle1 < 0:
                    cmd_vel_msg.angular.z = (math.pi + min_angle1) * 0.5
                else:
            ...
            if current_cmd_vel_msg.linear.x > 0.05 and -math.pi/4 < min_angle2 < math.pi/4:
                ...
                cmd_vel_msg.linear.x = current_cmd_vel_msg.linear.x * min_range2*2
                
                if min_angle2 > 0:
                    cmd_vel_msg.angular.z = (-math.pi + min_angle2) * 0.5
                elif min_angle2 < 0:
                    cmd_vel_msg.angular.z = (math.pi + min_angle2) * 0.5
                else:
                    cmd_vel_msg.angular.z = current_cmd_vel_msg.angular.z
            ...
            if current_cmd_vel_msg.linear.x > 0.05 and -math.pi/6 < min_angle3 < math.pi/6:
                ...
                cmd_vel_msg.linear.x = current_cmd_vel_msg.linear.x*min_range3*1.5

                if min_angle3 > 0:
                    cmd_vel_msg.angular.z = min((-math.pi + min_angle3) * 0.2, current_cmd_vel_msg.angular.z)
                elif min_angle3 < 0:
                    cmd_vel_msg.angular.z = max((math.pi + min_angle3) * 0.2, current_cmd_vel_msg.angular.z)
                else:
                    cmd_vel_msg.angular.z = current_cmd_vel_msg.angular.z
