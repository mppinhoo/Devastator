...
def generate_launch_description():
    ...
    zone_logger = Node(
        package=None,  # No package specified
        executable=os.path.join(script_directory, 'zone_logger.py'),  # Full path to the Python script
        name='zone_logger',
        output='screen'
    )
    ...
    nav2_nav_params = os.path.join(get_package_share_directory(package_name),'config','nav2_navfn_dwb.yaml')
    ...
    delayed_nav = TimerAction(period=4.0, actions=[nav2_nav])
    ...
    delayed_resource = TimerAction(period=5.0, actions=[resource_node])

    pose_data = Node(
        package=None,  # No package specified
        executable=os.path.join(script_directory, 'record_pose_to_csv.py'),  # Full path to the Python script
        name='pose_data',
        output='screen'
    )

    delayed_pose = TimerAction(period=1.0, actions=[pose_data])

    waypoint_logger = Node(
        package=None,  # No package specified
        executable=os.path.join(script_directory, 'waypoint_logger.py'),  # Full path to the Python script
        name='resource_check',
        output='screen'
    )

    delayed_waypoint = TimerAction(period=5.0, actions=[waypoint_logger])
